...
interface EventForSelectorCard {
  id: string;
  title: string;
  dateTime: Date;
  location: string;
...
interface EventSelectorCardProps {
  event: EventForSelectorCard;
  targetPath: string;
  locale: string;
}

export function EventSelectorCard({ event, targetPath, locale }: EventSelectorCardProps) {
  const t = useTranslations("events");
  const router = useRouter();
  const isRTL = locale === "he";

  const stats = event.stats || { total: 0, pending: 0, accepted: 0, declined: 0, totalGuestCount: 0 };
  ...
  const handleClick = () => {
    router.push(`/${locale}/dashboard/events/${event.id}/${targetPath}`);
  };
