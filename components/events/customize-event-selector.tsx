...
import { CustomizeEventCard } from "./customize-event-card";
import type { CustomizeEventData } from "@/actions/event-selector";

interface CustomizeEventSelectorProps {
...
export function CustomizeEventSelector({
  events,
  title,
  description,
  locale,
}: CustomizeEventSelectorProps) {
  ...
  if (events.length === 1) {
    router.push(`/${locale}/dashboard/events/${events[0].id}/customize`);
    return (
