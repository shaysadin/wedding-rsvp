...
import { AutomationsEventCard } from "./automations-event-card";
import type { AutomationsEventData } from "@/actions/event-selector";

interface AutomationsEventSelectorProps {
...
export function AutomationsEventSelector({
  events,
  title,
  description,
  locale,
}: AutomationsEventSelectorProps) {
  ...
  if (events.length === 1) {
    router.push(`/${locale}/dashboard/events/${events[0].id}/automation`);
    return (
