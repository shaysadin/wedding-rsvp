...
export function TemplateCreationDialogV3({
  ...
  const getDefaultVariables = () => {
    ...
    // Add {{12}} for Gift Payment URL (for EVENT_DAY templates)
    if (type === "EVENT_DAY") {
      vars["12"] = "קישור מתנה";
    }
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  משתנים זמינים: {`{{1}}`} שם אורח, {`{{2}}`} שם אירוע, {`{{3}}`} שם מקום, {`{{4}}`} כתובת, {`{{5}}`} תאריך, {`{{6}}`} שעה, {`{{7}}`} ניווט (Waze), {`{{8}}`} מספר שולחן, {`{{9}}`} הסעות, {`{{10}}`} תמונה, {`{{11}}`} RSVP, {`{{12}}`} קישור מתנה
                </p>
              </div>
