...
export const ColorPicker = ({
  ...
  const [hue, setHueInternal] = useState(initialColor.hue() || 0);
  const [saturation, setSaturationInternal] = useState(initialColor.saturationl() || 100);
  const [lightness, setLightnessInternal] = useState(initialColor.lightness() || 50);
  const [alpha, setAlphaInternal] = useState(initialColor.alpha() * 100);
  ...
  useEffect(() => {
    if (value) {
      try {
        ...
        setHueInternal(color.hue() || 0);
        setSaturationInternal(color.saturationl() || 100);
        setLightnessInternal(color.lightness() || 50);
        setAlphaInternal(color.alpha() * 100);
      } catch (e) {
  ...
  // Wrapper functions that also call onChange
